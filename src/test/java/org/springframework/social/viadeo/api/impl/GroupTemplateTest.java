...
public class GroupTemplateTest extends AbstractViadeoApiTest {
	...
	public void searchGroups() {
		mockServer.expect(requestTo("https://api.viadeo.com/search/groups?access_token=ACCESS_TOKEN&q=viadeo&limit=50"))
				.andExpect(method(GET)).andRespond(withResponse(jsonResource("testdata/group-search"), responseHeaders));
	...
	public void getGroupWithAndId() {
		mockServer.expect(requestTo("https://api.viadeo.com/agvhhkokasejibsVdfvxdquioO?access_token=ACCESS_TOKEN"))
				.andExpect(method(GET)).andRespond(withResponse(jsonResource("testdata/detailed-group"), responseHeaders));
